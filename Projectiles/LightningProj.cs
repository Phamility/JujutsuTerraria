...
namespace JujutsuTerraria.Projectiles
{
    ...
    public class LightningProj : ModProjectile
    {
        ...
        private bool CheckActive(Player owner)
        {
            ...
            {
                owner.ClearBuff(ModContent.BuffType<LightningBuff>());

                return false;
            }

            if (owner.HasBuff(ModContent.BuffType<LightningBuff>()))
            {
                Projectile.timeLeft = 2;
            }
            if (owner.ownedProjectileCounts[ModContent.ProjectileType<LightningProj>()] > 4)
            {
        ...
        private void GeneralBehavior(Player owner, out Vector2 vectorToIdlePosition, out float distanceToIdlePosition)
        {
            ...
            Vector2 idlePosition1 = owner.Center;
            
            Vector2 idlePosition2 = owner.Center;
            
            Vector2 idlePosition3 = owner.Center;
          
            Vector2 idlePosition4 = owner.Center;
     

            timer++;
            if (timer >= TimerValue)
            {
                TimerValue = Main.rand.Next(5, 15);
                Random2 = Main.rand.Next(0, 250);
                Random3 = Main.rand.Next(0, 250);
                timer = 0;
                Random = Main.rand.Next(1, 5);
            }
 
            idlePosition1.X = owner.Center.X - Random2;
            idlePosition1.Y = owner.Center.Y - Random3;
            idlePosition2.X = owner.Center.X + Random2;
            idlePosition2.Y = owner.Center.Y - Random3;
            idlePosition3.X = owner.Center.X + Random2;
            idlePosition3.Y = owner.Center.Y + Random3;
            idlePosition4.X = owner.Center.X - Random2;
            idlePosition4.Y = owner.Center.Y + Random3;
            if (Random == 1)
            {
                idlePosition = idlePosition1;
            }
            if (Random == 2)
            {
                idlePosition = idlePosition2;
            }
            if (Random == 3)
            {
                idlePosition = idlePosition3;
            }
            if (Random == 4)
            {
                idlePosition = idlePosition4;
            }
