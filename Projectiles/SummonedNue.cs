...
namespace TenShadows.Projectiles
{
    ...
    public class SummonedNue : ModProjectile
    {
        ...
        private bool CheckActive(Player owner)
        {
            ...
            {
                owner.ClearBuff(ModContent.BuffType<SummonNueBuff>());

                return false;
            }

            if (owner.HasBuff(ModContent.BuffType<SummonNueBuff>()))
            {
                Projectile.timeLeft = 2;
            }
            if (owner.ownedProjectileCounts[ModContent.ProjectileType<SummonedNue>()] > 1)
            {
